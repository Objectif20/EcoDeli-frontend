...
export default function ShipmentsListPage() {
    ...
    if (finished) {
      return (
        <Badge className="bg-green-100 text-green-800 hover:bg-green-100">
          {t("client.pages.office.shipment.onGoing.status.finished")}
        </Badge>
      );
    }
      ...
      case "pending":
        return (
          <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">
            {t("client.pages.office.shipment.onGoing.status.pending")}
          </Badge>
        );
      case "In Progress":
        return (
          <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">
            {t("client.pages.office.shipment.onGoing.status.inProgress")}
          </Badge>
        );
      default:
  ...
  const formatDate = (dateString: string) => {
    ...
    return isNaN(date.getTime())
      ? t("client.pages.office.shipment.onGoing.invalidDate")
  ...
  useEffect(() => {
    dispatch(
      setBreadcrumb({
        segments: [
          t("client.pages.office.shipment.onGoing.breadcrumb.home"),
          t("client.pages.office.shipment.onGoing.breadcrumb.shipments"),
        ],
  ...
  useEffect(() => {
    const fetchShipments = async () => {
      try {
        ...
        console.error(
          t("client.pages.office.shipment.onGoing.errorFetching"),
          error
        );
        setLoading(false);
      }
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-6">
          {t("client.pages.office.shipment.onGoing.title")}
        </h1>
      ...
      {!isStripeValidate && (
        <div className="mb-4">
          <div className="flex items-center bg-yellow-50 border border-yellow-200 rounded px-4 py-3 text-yellow-800">
            <MessageCircleWarning className="mr-2 w-5 h-5" />
            <span>
